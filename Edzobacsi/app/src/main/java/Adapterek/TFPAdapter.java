...
public class TFPAdapter extends RecyclerView.Adapter<TFPAdapter.MyViewHolder> implements Filterable {

    Context context;
    ToolsForPlaces tfp;
    private final ArrayList<String> tool_id_list;
    private final ArrayList<String> tool_name_list;
    private final ArrayList<String> checked_id_list;
    ...
    public TFPAdapter(Context context, ArrayList<String> id_list, ArrayList<String> name_list, ArrayList<String> checked_list) {
        this.context = context;
        this.tool_id_list = id_list;
        this.tool_name_list = name_list;
        this.checked_id_list = checked_list;
        this.tfp = (ToolsForPlaces) context;
    ...
    @NonNull
    @Override
    public TFPAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater inf = LayoutInflater.from(context);
        View view = inf.inflate(R.layout.row_items_with_check, parent, false);
        return new TFPAdapter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull TFPAdapter.MyViewHolder holder, int position) {
        holder.dontshow.setVisibility(View.GONE);
        holder.exercise_name.setClickable(false);
        holder.exercise_name.setFocusable(false);
        holder.exercise_name.setText(tool_name_list.get(position));
        ...
        holder.linOut.setOnClickListener(v -> {
            ...
            tfp.getCheckedItems(holder.exercise_name, position);
        });

        boolean checkedItem = false;
        for(int i = 0; i < checked_id_list.size(); i++) {
            if(Integer.parseInt(tool_id_list.get(position)) == Integer.parseInt(checked_id_list.get(i))){
                checkedItem = true;
                break;
            }
        }
